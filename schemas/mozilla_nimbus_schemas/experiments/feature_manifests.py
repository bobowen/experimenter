...
class SdkFeatureVariable(BaseFeatureVariable):
    ...
    def validate_enum(cls, data: Self) -> Self:
        if (
            data.enum is not None
            and data.type == FeatureVariableType.STRING
            and not all(isinstance(variant, str) for variant in data.enum)
        ):
...
class DesktopFeatureVariable(BaseFeatureVariable):
    ...
    enum: list[str] | list[int] | SkipJsonSchema[None] = Field(
        description=(
                ...
                "enum": {
                    "allOf": [
                        *(
                            ...
                            for ty in (
                                FeatureVariableType.STRING,
                                FeatureVariableType.INT,
                            )
                        ),
                        *(
                            {
    ...
    def validate_set_pref_fallback_pref_mutually_exclusive(cls, data: Self) -> Self:
        ...
        if has_set_pref and has_fallback_pref:
            raise ValidationError("fallback_pref and set_pref are mutually exclusive")
    ...
    def validate_enum(cls, data: Self) -> Self:
        if data.enum is not None:
            if data.type in (FeatureVariableType.STRING, FeatureVariableType.INT):
                ...
                raise ValueError("enums are only supported for string and integer types")
...
class BaseFeature(BaseModel):
    ...
    # This could be done declaratively by splitting the feature into
    model_config = ConfigDict(
        json_schema_extra={
            "if": {
                "properties": {
    ...
    def validate_exposure_description(cls, data: Self) -> Self:
        if data.has_exposure and data.exposure_description is None:
            raise ValidationError(
                "exposure_description is required if has_exposure is True"
            )
